...
final class Reimbursement {
    ...
    var receiptImageData: Data? // Store a compressed image blob (thumbnail-safe)

    init(id: UUID = UUID(), date: Date = .now, projectCode: String, note: String, status: ClaimStatus = .unclaimed, latitude: Double? = nil, longitude: Double? = nil, placeName: String? = nil, receiptImageData: Data? = nil) {
        self.id = id
        self.date = date
    ...
    func thumbnailImage(maxDimension: CGFloat = 600) -> UIImage? {
        guard let data = receiptImageData, let image = UIImage(data: data) else { return nil }
        let size = image.size
    ...
    func csvRow(delimiter: String = ",") -> String {
        ...
        let fields = [id.uuidString, dateStr, projectCode, note, status.rawValue, placeName ?? "", coordinateString]
...
struct AddEntryView: View {
    ...
    @State private var status: ClaimStatus = .unclaimed
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Receipt Screenshot") {
                    PhotosPicker(selection: $photoItem, matching: .images) {
                        HStack {
                            Image(systemName: "photo")
                            Text(receiptImage == nil ? "Pick screenshot" : "Change screenshot")
                        }
                    }
                    .onChange(of: photoItem) { old, newItem in
                        Task { receiptImage = try await newItem?.loadUIImageDownscaled() }
                    }
                ...
                Section("Details") {
                    ...
                    Picker("Status", selection: $status) {
                        ForEach(ClaimStatus.allCases) { s in Text(s.rawValue).tag(s) }
                    }
                }
                ...
                Section {
                    ...
                    .disabled(projectCode.trimmingCharacters(in: .whitespaces).isEmpty || receiptImage == nil)
                }
            }
    ...
    private func save() {
        ...
        let compressed = receiptImage?.jpegData(compressionQuality: 0.7)
...
struct ExportView: View {
    ...
    private func makeCSV() {
        let header = ["id","date","projectCode","note","status","placeName","coordinate"].joined(separator: ",")
...
// MARK: - Quick Actions (helpers for faster entry later)
...
struct QuickActionsView: View {
    ...
    var body: some View {
        Form {
            Section("Pasteboard → New Entry") {
                Button { createFromPasteboard() } label: { Label("Create from copied text + current time/location", systemImage: "doc.on.clipboard") }
                Text("Copies the current place + time; fill in screenshot later.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Section("Default Project Code") {
                TextField("Enter a project code to prefill on next create", text: $projectCode)
            }
        }
        .navigationTitle("Quick Actions")
    }

    private func createFromPasteboard() {
        let note = UIPasteboard.general.string ?? ""
        let r = Reimbursement(projectCode: projectCode.isEmpty ? "GEN" : projectCode, note: note)
        context.insert(r)
        try? context.save()
    }
}
